...
$SubscribedSKUs.value = $SubscribedSKUs.value | Where SkuId -NE '37c86dd4-ca27-4f07-8abc-fdf4a34d986c'
...
# Initialize the license report array.
$LicensesReport = @()

# Initialize the conditional text array.
$conditionalText = @()
...
foreach($sku in $SubscribedSKUs.value){
    ...
    # Export the license report array to Excel.
    $LicensesReport | Export-Excel -Path C:\Temp\TenantLicenseReport.xlsx -WorksheetName LicenseOverview -ConditionalText $conditionalText

    # Destroy license utilization variable.
    Remove-Variable licenseUtilization
    
    # Assemble and export users assigned to current license to Excel.
    "Assigned Users for: " + $LicensesHashTable.Item($sku.skuId) | Export-Excel -Path C:\Temp\TenantLicenseReport.xlsx -WorksheetName LicenseAssignmentsOverview -StartColumn $licenseAssignmentsStartColumn
    
    # Collect users that are assigned the current license.
    $users = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/v1.0/users?filter=assignedLicenses/any(x:x/skuId eq $skuGUID)&?select=id,displayName,userPrincipalName"
    ...
    foreach($user in $users.value){
        ...
        # Add user to array with display name and UPN.
        $licensedUsersArray += $user.displayName + " (" + $user.userPrincipalName + ")"
    }

    # Export licensed user array to Excel.
    $licensedUsersArray | Sort | Export-Excel -Path C:\Temp\TenantLicenseReport.xlsx -WorksheetName LicenseAssignmentsOverview -StartColumn $licenseAssignmentsStartColumn -StartRow 2
    
    # Increase license assignments start column by 1.
    $licenseAssignmentsStartColumn++
    
    # Initialize service plans array for current license.
    $servicePlans = @()

    #Assemble service plans array for current license.
    ...
    "(" + $LicensesHashTable.Item($sku.skuId) + ") Service Plans" | Export-Excel -Path C:\Temp\TenantLicenseReport.xlsx -WorksheetName ServicePlansOverview -StartRow 1 -StartColumn $servicePlansStartColumn
    $servicePlans | Export-Excel -Path C:\Temp\TenantLicenseReport.xlsx -WorksheetName ServicePlansOverview -StartRow 3 -StartColumn $servicePlansStartColumn
