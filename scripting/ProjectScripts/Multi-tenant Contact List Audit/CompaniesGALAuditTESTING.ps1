...
Connected to MSOnline for:" ($client.OrgDisplayName).ToString()
            ...
            $UsersFailedToAddAsContactInOtherTenantsPath = ($currentClient).ToString() + "_Users_FailedToAddAsContactIn_" + ($client.OrgDisplayName).ToString() + ".csv"
            $UsersAddedAsContactInOtherTenantsPath = ($currentClient).ToString() + "_Users_AddedAsContactIn_" + ($client.OrgDisplayName).ToString() + ".csv"
...
User" $user.DisplayName "("$user.EmailAddress")" "does not exist in" $client.OrgDisplayName "as a mail contact and will be added."
                    ...
                    if($Error.Exception -Match "Ex55FBCD"){ # This curly bracket opens the if/else statement that checks the error reason and sets the error code accordingly.
                        ...
                        $ErrorCode = "600"
                        $ErrorDescription = "The users primary SMTP address is being used by another object somewhere in Azure AD or Exchange."
                    }

                    elseif($Error.Exception -Match "Ex1472D1"){

                        $ErrorCode = "601"
                        $ErrorDescription = "User exists as a guest user in Azure AD/Entra ID, and subsequently cannot be created as a mail contact."
                    }
...
Confirmed the user" $user.DisplayName "("$user.EmailAddress")" "was added as a mail contact in" $client.OrgDisplayName "and the script will now move on to the next user."
                        ...
                        $user.DisplayName + "," + $user.EmailAddress | Out-File $UsersAddedAsContactInOtherTenantsPath
                    }
...
Failed to add" $user.DisplayName "("$user.EmailAddress")" "as a mail contact in" $client.OrgDisplayName "please add manually via the Exchange Admin Center."
                        ...
                        $user.DisplayName + "," + $user.EmailAddress + "," + $ErrorCode + "," + $ErrorDescription | Out-File $UsersFailedToAddAsContactInOtherTenantsPath -Append
                    }
...
Reports of" $currentClient.ToString() "users that are not contacts in" $client.OrgDisplayName "are now being converted to delimited format...."
            ...
            # This only works for the NonContactInOtherTenants reports at the moment.
...
A report of" $currentClient "users that were added as contacts to the global address list for:" $client.OrgDisplayName "was generated at:" $FullUsersAddedAsContactInOtherTenantsPath
                ...
                Write-Host "
No" $currentClient "users were missing as contacts from the global address list for" $client.OrgDisplayName "and therefore none were added."
            }
