# This script was designed to be used in a Microsoft Partner tenant and report against customer tenants.

$DateTime = Get-Date -Format "M-dd-yyyy-hmmtt"

$CustomerList = Import-CSV ".\Path\To\CustomersCSVFileHere.csv"
$CustomerCounter = 1
...
$ReportPath = "Path\Reports\$ClientName\SPF_DKIM_DMARC_Reports"
...
DKIM configuration/DKIM keys not created for: $DomainDisplayName"
        ...
        $DMARCRecordValue = $DMARCConfigurationCheck.Strings

        if($DMARCConfigurationCheck -EQ $null){
        ...
        elseif($DMARCConfigurationCheck -NE $null){
            ...
            "DMARC Value: $DMARCRecordValue" | Export-Excel -Path ".\SPF_DKIM_DMARC_Report-$DateTime.xlsx" -WorksheetName "Domain $DomainCounter" -StartColumn 1 -StartRow 11
